...
app.get("/api/foods", (req, res) => {
    ...
    fs.readFile(DATA_FILE, (err, data) => {
      ...
      if (query) {
        ...
        console.log("No result found with matching query");
        res.json(matchedFoods);
      }      
    });
  });
...
app.put("/api/foods", (req, res) => {
    ...
    app.delete("/api/foods", (req, res) => {
      ...
      fs.readFile(DATA_FILE, (err, data) => {
          ...
          // Force all ids to be strings for consistent comparison
          const deleteId = String(req.body.id);
  
          foods = foods.reduce((memo, food) => {
              if (String(food.id) === deleteId) {
